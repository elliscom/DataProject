...
GK_data = data_filtered[data_filtered['position'] == 'GK'][GK_stats]

GK_data.to_csv("GK_DATA.csv")
files.download("GK_DATA.csv")
...
GK_data = pd.read_csv(list(uploaded.keys())[0], index_col=0)

# This handles missing values.
numeric_cols = GK_data.select_dtypes(include=np.number).columns.tolist()
GK_data[numeric_cols] = GK_data[numeric_cols].fillna(GK_data[numeric_cols].median())

# Define training features, x and target y
X = GK_data.drop(['total_points'], axis=1) #drop total_points (for predictions)
...
GK_df = GK_df[GK_df['position'] == 'GK']

GK_df = GK_df[GK_df['AV_minutes_past5'] > 10] #get rid of players who have no played an average of over 10 mins over 5 games. 

GK_df = GK_df.sort_values(by='predicted_total_points', ascending=False)

GK_df = GK_df.drop(columns=['AV_minutes_past5'])
