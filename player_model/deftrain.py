...
DEF_data = data_filtered[data_filtered['position'] == 'DEF'][DEF_stats]

DEF_data.to_csv("DEF_DATA.csv")
files.download("DEF_DATA.csv")
...
DEF_data = pd.read_csv(list(uploaded.keys())[0], index_col=0)

# This handles missing values.
numeric_cols = DEF_data.select_dtypes(include=np.number).columns.tolist()
DEF_data[numeric_cols] = DEF_data[numeric_cols].fillna(DEF_data[numeric_cols].median())

# Define training features, x and target y
X = DEF_data.drop(['total_points'], axis=1) #drop total_points (for predictions)
...
DEF_df = DEF_df[DEF_df['position'] == 'DEF']

DEF_df = DEF_df[DEF_df['AV_minutes_past5'] > 10] #get rid of players who have no played an average of over 10 mins over 5 games. 

DEF_df = DEF_df.sort_values(by='predicted_total_points', ascending=False)

DEF_df = DEF_df.drop(columns=['AV_minutes_past5'])
