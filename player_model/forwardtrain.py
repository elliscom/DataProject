...
FWD_data = data_filtered[data_filtered['position'] == 'FWD'][FWD_stats]

FWD_data.to_csv("FWD_DATA.csv")
files.download("FWD_DATA.csv")
...
FWD_data = pd.read_csv(list(uploaded.keys())[0], index_col=0)

# This handles missing values.
numeric_cols = FWD_data.select_dtypes(include=np.number).columns.tolist()
FWD_data[numeric_cols] = FWD_data[numeric_cols].fillna(FWD_data[numeric_cols].median())

# Define training features, x and target y
X = FWD_data.drop(['total_points'], axis=1) #drop total_points (for predictions)
...
fwd_df = fwd_df[fwd_df['position'] == 'FWD']

fwd_df = fwd_df[fwd_df['AV_minutes_past5'] > 10] #get rid of players who have no played an average of over 10 mins over 5 games. 

fwd_df = fwd_df.sort_values(by='predicted_total_points', ascending=False)

fwd_df = fwd_df.drop(columns=['AV_minutes_past5'])
